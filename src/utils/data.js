export const initialCards = [
  {
    id: 0,
    title: 'Национальное достояние – паркиб ациональное достояние – парки',
    text: 'В 2016 году Америка отмечала важный юбилей: сто лет назад здесь начала складываться система национальных парков – охраняемых территорий, где и сегодня каждый может приобщиться к природе.',
    source: 'лента.ру',
    keyword: 'Природа',
    date: '2 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c1.wallpaperflare.com/preview/287/1019/785/blue-green-leaves-plants.jpg'
  },
  {
    id: 1,
    title: 'Лесные огоньки: история',
    text: 'Фотограф отвлеклась от освещения  Америка отмечала важный юбилей: сто лет назад здесь начала складываться система наци суровой политической реальности Мексики, чтобы запечатлеть ускользающую красоту одного из местных чудес природы.',
    source: 'медуза',
    keyword: 'Фотография',
    date: '3 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c1.wallpaperflare.com/preview/297/539/696/plant-leaf-minimal-botanical.jpg'
  },
  {
    id: 2,
    title: 'Первозданная тайга»: новый фотопроект Игоря Шпиленка Игоря Шпиленка',
    text: 'Фотограф отвлеклась от освещения  Америка отмечала важный юбилей: сто лет назад здесь начала складываться система наци суровой политической реальности Мексики чтобы запечатлеть ускользающую красоту одного из местных чудес природы.',
    source: 'лента.ру',
    keyword: 'Природа',
    date: '4 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c1.wallpaperflare.com/preview/419/429/240/leaf-plant-eucalyptus-green.jpg'
  },
  {
    id: 3,
    title: 'Национальное достояние',
    text: 'В 2016 году Америка отмечала важный юбилей: сто лет назад здесь начала складываться система национальных парков – охраняемых территорий, где и сегодня каждый может приобщиться к природе.',
    source: 'медуза',
    keyword: 'Тайга',
    date: '5 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c4.wallpaperflare.com/wallpaper/937/369/210/leaves-water-drops-blurred-photography-wallpaper-preview.jpg'
  },
  {
    id: 4,
    title: 'Лесные огоньки: история',
    text: 'Знаменитый фотограф снимает первозданные леса России, чтобы рассказать о  Америка отмечала важный юбилей: сто лет назад здесь начала складываться система наци необходимости их сохранения. В этот раз он отправился в Двинско-Пинежскую тайгу, где.',
    source: 'лента.ру',
    keyword: 'Фотография',
    date: '6 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c4.wallpaperflare.com/wallpaper/1021/8/61/rain-nature-wood-landscape-wallpaper-preview.jpg'
  },
  {
    id: 5,
    title: 'Первозданная тайга»: новый фотопроект Игоря Шпиленка',
    text: 'Знаменитый фотограф снимает первозданные леса России, чтобы рассказать о  Америка отмечала важный юбилей: сто лет назад здесь начала складываться система наци необходимости их сохранения. В этот раз он отправился в Двинско-Пинежскую тайгу, где.',
    source: 'лента.ру',
    keyword: 'Тайга',
    date: '4 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c4.wallpaperflare.com/wallpaper/752/37/124/succulent-nature-plants-flowers-wallpaper-preview.jpg'
  },
  {
    id: 6,
    title: 'Национальное достояние – парки',
    text: 'В 2016 году Америка отмечала важный юбилей: сто лет назад здесь начала складываться система национальных парков – охраняемых территорий, где и сегодня каждый может приобщиться к природе.',
    source: 'медуза',
    keyword: 'Фотография',
    date: '5 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c4.wallpaperflare.com/wallpaper/937/369/210/leaves-water-drops-blurred-photography-wallpaper-preview.jpg'
  },
  {
    id: 7,
    title: 'Лесные огоньки: история',
    text: 'Знаменитый фотограф снимает первозданные леса России, чтобы рассказать о необходимости их сохранения. В этот раз он отправился в Двинско-Пинежскую тайгу, где.',
    source: 'лента.ру',
    keyword: 'Природа',
    date: '6 августа, 2019',
    link: 'https://praktikum.yandex.ru',
    url: 'https://anupamchugh.medium.com/3-reasons-why-the-bitcoin-bull-run-of-2020-is-different-from-2017-550f4e3e255e',
    image: 'https://c4.wallpaperflare.com/wallpaper/1021/8/61/rain-nature-wood-landscape-wallpaper-preview.jpg'
  }
];